    ...
    public void BuildRateMap()
        {
            ...
            //Bitmap bmpRate = new Bitmap(fileAndDirectory);
            // Lock the bitmap's bits.  
            Rectangle rect = new Rectangle(0, 0, bmpRate.Width, bmpRate.Height);
            System.Drawing.Imaging.BitmapData bmpData =
                bmpRate.LockBits(rect, System.Drawing.Imaging.ImageLockMode.ReadWrite,
                bmpRate.PixelFormat);
            ...
            bmpWidth = bmpRate.Width;
            bmpHeight = bmpRate.Height;

            //24 bits per pixel - 3 bytes format is BGR
            byte[] redBytes = new byte[(int)(bmpWidth * bmpHeight)];
            byte[] flipRed = new byte[(int)(bmpWidth * bmpHeight)];

            ////extract the red values
            int rc = 0;
            for (var r = 0; r < bmpHeight; r++) //each scanline row
            {
                for (var c = 2; c < bmpWidth * 3; c +=3) //each red pixel
                {
                    redBytes[rc] = rgbValues[(r * bmpData.Stride) + c]; //remove padding bytes
                    rc++;
                }
            }

            //flip the image top to bottom
            for (int i = 0, j = redBytes.Length - bmpWidth; i < redBytes.Length; i += bmpWidth, j -= bmpWidth)
            {
                for (int k = 0; k < bmpWidth; ++k)
                {
                    flipRed[i + k] = redBytes[j + k];
                }
            }

            int bmpPixelCount = flipRed.Length;

            double fieldHeight = (mf.maxFieldY - mf.minFieldY); //height
            double fieldWidth = (mf.maxFieldX - mf.minFieldX); //width
            ...
            if (rateMapScale < 8) rateMapScale = 8; //blocks no smaller then 8 meters
            //rateMapScale = 4;
            ...
            double bmpToGridScaleWidth = (double)bmpWidth / (double)gridWidth;
            double bmpToGridScaleHeight = (double)bmpHeight / (double)gridHeight;

            //the grid with northing and easting from actual field
            vec3[,] gridArr = new vec3[gridHeight, gridWidth];
            ...
            for (int i = 0; i < bmpHeight; i++)
            {
                ...
                {
                    bmpArr[i, j] = flipRed[(i * bmpWidth) + j];
                }
            }
            ...
            for (int i = 0; i < gridHeight; i++)
            {
                ...
                {
                    gridArr[i, j].easting = (j * rateMapScale) + (int)mf.minFieldX;
                    gridArr[i, j].northing = (i * rateMapScale) + (int)mf.minFieldY;

                    gridArr[i, j].heading = (double)(bmpArr[(int)(i * bmpToGridScaleHeight), (int)(j * bmpToGridScaleWidth)]);
                    mapList.Add(gridArr[i, j]);
                }
            }
        ...
        public void DrawArr()
        {
            ...
            GL.Color3(0.440425f,0.425f,0.40425f);
            GL.BindTexture(TextureTarget.Texture2D, mf.texture[4]);
            ...
            //{
            //    GL.Color3((byte)mapList[i].heading, (byte)0, (byte)0);
